...
| `Gx.7` behavior | module + group tests, warning gate, browser/console smoke, each its own run. Smoke is **first-click every group menu** and the first tab of every designer/wizard the contract names (calendar, kanban, Filters). A compile-time inherit miss or `undefined.bind` / `undefined.collapsed` is a Gx.3 miss, not a walk finding. **Then launch the review DB** (`gx8_<group>`, 19.0 loader counts, `en_US`, `admin`/`admin` + `demo`/`demo`) and **hand over the URL**. A Gx.7 that only has test logs has not finished. |
